  ...
  return (
    <nav className="w-full bg-[#00743F] text-white sticky top-0 z-30 shadow">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 py-3">
        <span className="font-bold text-lg tracking-wide">Justus Chicks</span>
        ...
        <ul className="hidden sm:flex gap-6 font-medium">
          {navLinks.map(link => (
            <li key={link.name}>
              <Link href={link.href} className="hover:underline underline-offset-4">{link.name}</Link>
            </li>
          ))}
        </ul>
      </div>
      {open && (
        <ul className="sm:hidden flex flex-col gap-2 px-4 pb-3 bg-[#00743F]">
          {navLinks.map(link => (
            <li key={link.name}>
              <Link href={link.href} className="block py-1 hover:underline underline-offset-4">{link.name}</Link>
            </li>
          ))}
        </ul>
      )}
